...
export const findTriggersToUpdate = (
  ...
  const triggersToUpdated = parsedTriggers.reduce(
    (list: TriggerParsedType[], current: TriggerParsedType) => {
      const trigger = deployTriggers.find(
        (trigger) =>
          trigger.name === current.name &&
          signTrigger(deepPick(trigger, compareFields)) !==
            signTrigger(deepPick(current, compareFields))
      );
      if (trigger) {
