  ...
  try {
    ...
    const validateSchema = z.array(
      z.object({
        ...
        operation_types: z.array(
          z.string({ required_error: "operation_types item is required" }),
        ...
        match: z.object(
          ...
          {
            required_error:
              "match pattern is required, event if an empty object",
          }
        ),
      })
    );
    ...
    const token = await authenticate(
      String(args.mongodb_api_key),
      String(args.mongodb_api_secret)
    );

    // get all created triggers on atlas realm
    console.log("Fetching current triggers");
    const onlineTriggers = await findAllTriggers(
      token,
    ...
    const triggersToCreate = findTriggersToCreate(
      onlineTriggers,
      currentTriggers
    );
    ...
    const triggersToDelete = findTriggersToDelete(
      onlineTriggers,
      currentTriggers
    );
    ...
    const triggersToUpdate = findTriggersToUpdate(
      onlineTriggers,
      currentTriggers
    );
    ...
    const allTriggers = await findAllTriggers(
      token,
      String(args.mongodb_group_id),
      String(args.mongodb_app_id)
    );
    ...
    if (error instanceof ZodError) {
      ...
      console.error(
        JSON.parse(error.message).reduce((errors, error) => {
