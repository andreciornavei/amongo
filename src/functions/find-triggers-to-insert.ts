...
export const findTriggersToCreate = (
  ...
  const triggersToCreate = deployTriggers.reduce((list: TriggerParsedType[], current: TriggerParsedType) => {
    if (!onlineTriggers.some((trigger) => trigger.name === current.name)) list.push(current);
    return list;
  }, []);
